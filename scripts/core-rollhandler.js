...
Hooks.once('tokenActionHudCoreApiReady', async (coreModule) => {
    SavageRollHandler = class SavageRollHandler extends coreModule.api.RollHandler {
        ...
        async doHandleActionEvent(event, encodedValue) {
            const module = game.modules.get("swade-tools")
            console.log(module)
            if (module && module.active) {
                // Module is available and active, use its functions
                //const moduleFunction = new module.exports.SystemRoll(this.actor);
                const moduleFunction = module.exports
                // Call the function
                console.log(moduleFunction);
            } else {
               // Module is not available or active
               console.log("Module is not available or active.");
            }
            /*if (game.modules.get("swade-tools")?.active) {
                let test = new swadeTools.SystemRoll(this.actor)
                console.log(test)
            }*/
